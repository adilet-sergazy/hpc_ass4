...
int main(int argc, char **argv) {
    ...
    int lastrow = viewPlaneResolutionX % size
    if (rank == (size - 1)) {
  mySizeX = mySizeX + lastrow;
    }
	...
	for (int src_rank = 0; src_rank < size; src_rank++) {
            ...
            int lastRowsX = 0;
            if (src_rank == (size - 1)){
                lastRowsX = viewPlaneResolutionX % size;
            } else{
                lastRowsX = 0;
            }
            int srcSizeY = mySizeY;
            Image srcPiece(srcSizeX + lastRowsX, srcSizeY);
            MPI_Recv(srcPiece.getData(), (srcSizeX+lastRowsX) * srcSizeY * 3, MPI_DOUBLE, src_rank, 0, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
          for (int x = 0; x < srcSizeX + lastRowsX; x++)
